...
// Issued on every read rather than latched behind a flag: the board is cached at the edge, so
// this reaches the database about once every ten seconds, and no hidden state means it cannot
// quietly stop happening. A failure is ignored; the next read tries again.
const purgeDemoRows = () =>
  sb('purchases?paypal_order_id=like.DEMO-*', { method: 'DELETE' })
    .catch(err => console.warn('[zones] demo row purge failed, will retry', err.message));
